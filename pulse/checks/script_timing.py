"""pulse/checks/script_timing.py — read autobuild run_all results, track
...
State (per Pulse instance):
- ~/.pyauto-pulse/timings/<workspace>__<dir>__<file>.json
  containing a rolling window of recent durations.

Output:
- ~/.pyauto-pulse/script_timing.json with the latest regression summary.
...
def update_history(slug: str, duration: float, window: int) -> list[float]:
    ...
    PULSE_TIMINGS_DIR.mkdir(parents=True, exist_ok=True)
    history_path = PULSE_TIMINGS_DIR / slug
...
def run(results_dir: Path | None = None) -> dict[str, Any]:
    ...
    (PULSE_STATE_DIR / "script_timing.json").write_text(json.dumps(summary, indent=2))
    return summary
...
def main(argv: list[str]) -> int:
    ...
    from pulse_color import c_ok, c_warn, c_fail, c_info, c_meta, glyph_ok, glyph_warn, glyph_fail
