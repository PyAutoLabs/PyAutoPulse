#!/usr/bin/env bash
# pulse/checks/verify_install.sh — deep install-readiness check for PyAutoLens.
#
# Owned by PyAutoPulse (moved from PyAutoBuild): all release-readiness checking
# is Pulse's job. This is a *deep, on-demand* check — it creates throwaway venvs
# / conda envs and installs from PyPI, so it takes minutes and must NEVER run in
# the <30s `tick` loop. Invoke it via `pyauto-pulse verify_install`, which passes
# `--report-json $PULSE_STATE_DIR/verify_install.json`; readiness then consumes
...
Options:
  ...
  --report-json PATH  Also write a machine-readable {ts,ready,version,checks}
                      JSON sidecar to PATH (consumed by pyauto-pulse readiness).
...
# ----- machine-readable sidecar (consumed by pyauto-pulse readiness) -----
