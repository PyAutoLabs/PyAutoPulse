#!/usr/bin/env bash
# pulse/checks/url_sweep.sh — offline URL-hygiene sweep across the PyAuto repos.
#
# Runs pulse/checks/url_check.sh (the offline forbidden-pattern regex guard) over
# every repo that used to carry its own .github/workflows/url_check.yml, and
# aggregates the results into $PULSE_STATE_DIR/url_check.json:
#
...
# surfaced in `pyauto-pulse status` but does NOT feed `readiness`. It runs
# on-demand (`pyauto-pulse url_sweep`) and from the central url-check.yml cloud
...
check_url_sweep() {
  pulse_state_dir
  pulse_log INFO "$(c_info "url_sweep: scanning ${#URL_CHECK_REPOS[@]} repos for forbidden URL patterns")"
...
')"
  ...
  pulse_write_json "$PULSE_STATE_DIR/url_check.json" "$result_json"
