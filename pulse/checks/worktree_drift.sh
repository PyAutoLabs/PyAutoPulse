#!/usr/bin/env bash
# pulse/checks/worktree_drift.sh — PyAutoLabs-wt/ dirs vs active.md claims.
#
# Three signals:
...
#
# Writes $PULSE_STATE_DIR/worktree_drift.json with the categorised lists.
# Colour summary:
#   no drift          → green
...
check_worktree_drift() {
  pulse_state_dir
  pulse_log INFO "$(c_info "worktree_drift: scanning $PYAUTO_WT_ROOT vs $ACTIVE_MD")"
...
PY
  ...
  pulse_write_json "$PULSE_STATE_DIR/worktree_drift.json" "$result_json"
  ...
  pulse_log OK "$(c_ok "worktree_drift: done")"
}
